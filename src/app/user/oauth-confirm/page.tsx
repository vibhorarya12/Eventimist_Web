...
import { useUserOAuth } from '@/hooks/eventimist/user/auth/useUserOAuth';
import { useUserAuth } from '@/store/eventimist/user/auth/UserAuthState';
import { useClerk } from '@clerk/nextjs';
import { useRouter, useSearchParams } from 'next/navigation';

export default function UserOauthConfirm() {
  ...
  const { oauthLogin, oauthRegister, loading, error, reset } = useUserOAuth();

  // ─── Check if user is already authenticated ───────────────────────────────────
  // If user/session exist AND user auth data exists, redirect to profile
  ...
  const handleConfirm = async () => {
    if (purpose === 'signup') {
      ...
      await oauthLogin({ email: email!, clerkSessionId: clerkSessionID! });
    }
  };

  const heading = purpose === 'signup' ? 'Create your user account' : 'Welcome back';
      ...
      <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-sm">
          ...
          <div className="fade-up d1 flex items-center justify-center gap-2.5 mb-8">
            <div className="w-9 h-9 rounded-xl bg-linear-to-br from-amber-400 to-orange-500 flex items-center justify-center shadow-lg shadow-amber-400/30">
            ...
            <div>
              ...
              <div className="text-amber-600 text-[9px] font-bold tracking-widest mt-0.5">USER</div>
            </div>
          </div>
            ...
            <div className="px-8 pt-8 pb-8">
              ...
              <div className="fade-up d2 flex justify-center mb-6">
                <div className="relative">
                  ...
                  {/* Verified badge */}
                  <div className="absolute -bottom-2 -right-2 w-7 h-7 rounded-xl bg-linear-to-br from-amber-400 to-orange-500 flex items-center justify-center shadow-md">
              ...
              {error && (
                <div className="fade-up mb-5 flex items-start gap-3 bg-rose-50 border border-rose-200 text-rose-600 text-sm px-4 py-3 rounded-xl">
                  ...
                  <button onClick={reset} className="text-rose-400 hover:text-rose-600 transition-colors shrink-0">
              ...
              <div className="fade-up d4 space-y-3">
                ...
                <button
                  onClick={async () => await signOut({ redirectUrl: '/user/auth' })}
                  disabled={loading}
