...
function PanelCarousel() {
  ...
  const next = useCallback(() => goTo((current + 1) % USER_SLIDES.length), [current, goTo]);
    ...
    <div className="relative w-full h-full overflow-hidden">
      ...
      <div className="absolute top-1/2 -translate-y-1/2 left-8 right-8 z-10">
        <div className="bg-white/10 backdrop-blur-md border border-white/15 rounded-2xl p-5 max-w-xs">
          ...
          <p className="text-white/90 text-sm leading-relaxed italic">
            "Found three amazing local events in my first week. The map view makes discovering events so easy!"
          </p>
          <div className="flex items-center gap-2 mt-3">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-xs font-bold text-white">A</div>
            <div>
              <div className="text-white text-xs font-bold">Anjali Mehta</div>
              <div className="text-white/50 text-[10px]">Community Member, Pune</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 p-10">
        ...
        <p key={current + "-stat"} className="slide-up text-white/60 text-sm mb-8" style={{animationDelay:"120ms"}}>
          {USER_SLIDES[current].stat}
        </p>

        {/* Stats row */}
        <div className="flex items-center gap-6 mb-6">
          {[{ val:"50K+", label:"Events" },{ val:"120K+", label:"Members" },{ val:"40+", label:"Cities" }].map(s => (
            <div key={s.label} className="text-center">
              <div className="text-white font-black text-xl" style={{fontFamily:"'Playfair Display',Georgia,serif"}}>{s.val}</div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-white/40 text-xs font-mono">
            ...
            <span>/</span>
            <span>{String(USER_SLIDES.length).padStart(2,"0")}</span>
          </div>
        </div>
...
function AuthPanel() {
  ...
  // const [city, setCity] = useState(""); // kept for future use
  // const [interests, setInterests] = useState<string[]>([]); // kept for future use
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .carousel-progress { animation: carousel-progress 5s linear forwards; }
      ...
      <div className="min-h-screen flex flex-col lg:flex-row bg-white">
        ...
        <div className="flex-1 flex flex-col min-h-screen overflow-y-auto">
          ...
          <div className="lg:hidden h-52 relative mx-6 mt-4 rounded-3xl overflow-hidden shadow-lg flex-shrink-0">
            <img src={USER_SLIDES[0].url} alt="Events" className="w-full h-full object-cover"/>
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-5">
              <div>
                <div className="text-amber-400 text-xs font-bold tracking-widest uppercase mb-1">User Portal</div>
                <div className="text-white text-lg font-black" style={{fontFamily:"'Playfair Display',Georgia,serif"}}>Discover Events Near You</div>
              </div>
            </div>
          </div>
